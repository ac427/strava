...
for a_id in A_IDS:
    ...
    if a_stream.distance is not None and a_stream.distance[-1] >= 0.00062137 :
        ...
        for i in range(math.floor(total_distance)):
            ...
            a_velocity_smooth.append(a_stream.velocity_smooth[index])
            try:
        ...
        META[a_id] = {"date": a_date, "mile_time": time_per_mile, "time": a_time,
                      "heartrate": a_heartrate, "velocity_smooth": a_velocity_smooth, "altitude": a_altitude}
...
AXS[1, 0].set_title('velocity_smooth')
for values in META.values():
    AXS[1, 0].plot(range(len(values['velocity_smooth'])),
                   values['velocity_smooth'], label=values['date'])
